...
public class SortableDoublePair implements Comparable<SortableDoublePair>{
  ...
  @Override public int compareTo(SortableDoublePair o) {
    double $ = Math.sqrt(Math.pow(unbox.unbox(Utils.cantBeNull(pair.first)),2)
                              + Math.pow(unbox.unbox(Utils.cantBeNull(pair.second)),2));
    double bDist = Math.sqrt(Math.pow(unbox.unbox(Utils.cantBeNull(o.getPair().first)),2)
        + Math.pow(unbox.unbox(Utils.cantBeNull(o.getPair().second)),2));
