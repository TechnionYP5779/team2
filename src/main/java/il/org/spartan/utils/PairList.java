...
public class PairList {
  ...
  public double distSum() {
    ...
    for(int i = 0; i < list.size(); i++) {
      sum += Math.sqrt(Math.pow(unbox.unbox(Utils.cantBeNull(this.at(i).first)),2)
          + Math.pow(unbox.unbox(Utils.cantBeNull(this.at(i).second)),2));
    }
    return sum;
  }
  ...
  public double sum() {
    double sum = 0;
    for(int i = 0; i < list.size(); i++) {
      sum += unbox.unbox(Utils.cantBeNull(this.at(i).first)) + 
          unbox.unbox(Utils.cantBeNull(this.at(i).second));
    }
    return sum;
  }
